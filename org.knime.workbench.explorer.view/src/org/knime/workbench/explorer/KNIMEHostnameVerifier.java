...
/**
 * Hostname verifier that accepts all hostnames for the KNIME standard certificate.
 *
 * @author Thorsten Meinl, KNIME.com, Zurich, Switzerland
 */
class KNIMEHostnameVerifier implements HostnameVerifier {
    /**
     * Singleton instance.
     */
    public static final HostnameVerifier INSTANCE = new KNIMEHostnameVerifier();

    private final HostnameVerifier m_defaultVerifier = new BrowserCompatHostnameVerifier();

    private KNIMEHostnameVerifier() {
    }
        ...
        try {
            return m_defaultVerifier.verify(hostname, session)
                || session.getPeerCertificateChain()[0].getSubjectDN().toString()
                    .equals("CN=default-server-installation.knime.local, O=KNIME.com AG, L=Atlantis, ST=Utopia, C=AA");
