...
public class SVGExportWizard extends ExportWizard
    implements IExportWizard {
	
	private List<ConnectionContainerEditPart> drawnConnections = new LinkedList<ConnectionContainerEditPart>();
    ...
    public boolean performFinish() {
    	...
    	if (fileDestination != null && fileDestination.trim().length() > 0) {
            if (fileDestination.length() < 5
                    || fileDestination.lastIndexOf('.') < fileDestination.length() - 4) {
            	fileDestination += ".svg";
            }
		...
		try {
			if (m_page.embedData()){
			    ...
			    iHandler =new CachedImageHandlerPNGEncoder(outFile.getParent(), outFile.getParent());
			}
        ...
        WorkflowEditor editor = (WorkflowEditor)PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor();
    	...
    	// Obtain the bounds of the workflow by searching for the biggest existing x and y values of the painted Figures. Add nodewidth + 10px for good measure.
    	// Also obtain the minimum x and y values used in the Workflow and pass them on to the wrapper, since Rectangles with negative x and y can't be created.
    	...
    	BatikGraphics2DWrapper graphics = new BatikGraphics2DWrapper(PlatformUI.getWorkbench().getDisplay(), ctx, false, xOffset, yOffset, bounds);
    	...
    	// Now we need to paint the connections. In order to do this we get all nodes to which the user may add connections and draw these
    	...
    	for (Object child : part.getChildren()){
    		...
    		for (ConnectionContainerEditPart connection : ((NodeContainerEditPart)child).getAllConnections()){
    		...
    		if (child instanceof SubworkflowEditPart){
        		for (ConnectionContainerEditPart connection : ((SubworkflowEditPart)child).getAllConnections()){
        			if (!drawnConnections.contains(connection)){
        				connection.getFigure().paint(graphics);
              			graphics.resetColors();
              			drawnConnections.add(connection);
